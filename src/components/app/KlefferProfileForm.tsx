        ...
        {state.favorite_games.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {state.favorite_games.map((g) => (
              ...
              >
                {g.imageUrl ? (
                  <img width={24} height={24} loading="lazy" decoding="async" src={g.imageUrl} alt="" className="h-6 w-6 rounded-full object-cover" />
                ) : null}
                {g.name}
                <button
                  type="button"
            ...
            <ul className="absolute z-20 mt-1 w-full max-h-64 overflow-auto rounded-xl border border-ink/20 bg-card shadow-lg">
              {results.map((g) => (
                <li key={g.id}>
                  ...
                  >
                    {g.imageUrl ? <img width={32} height={32} loading="lazy" decoding="async" src={g.imageUrl} alt="" className="h-8 w-8 rounded object-cover" /> : null}
                    {g.name}
                  </button>
                </li>
              ))}
