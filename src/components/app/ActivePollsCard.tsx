  ...
  return (
    <section className="bg-card border-2 border-ink rounded-2xl p-6 shadow-tactile-sm space-y-3">
      <header className="flex items-center gap-2">
        ...
        <h2 className="font-display text-xl font-bold">Tu voz cuenta</h2>
      </header>
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {p.kind === "survey" ? "Encuesta" : "Votación de adquisiciones"}
                  {p.closesAt
                    ? ` · cierra el ${new Date(p.closesAt).toLocaleDateString("es-ES", { day: "numeric", month: "short" })}`
                    : ""}
                </p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {p.participated ? (
                  <Badge variant="secondary">Participaste</Badge>
                ) : (
                  <Badge className="bg-coral-deep text-cream">Pendiente</Badge>
                )}
