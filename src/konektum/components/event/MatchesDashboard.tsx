...
import { Avatar, AvatarFallback } from "@/konektum/ui/avatar";
import { 
  Heart, 
  Users, 
  Sparkles, 
...
interface Match {
  ...
  /** True when the match derives from a Super Like */
  viaSuper?: boolean;
  /** True when the match comes from an accepted Flechazo */
  viaCrush?: boolean;
}
...
type MatchType = "dating" | "friendship" | "both" | "super" | "crush";
...
const MatchesDashboard = ({ matches, selections, participants, eventName, eventStatus, onShowQR, onRefresh, isProfessional = false }: MatchesDashboardProps) => {
  ...
  // Group matches by type ("super" = derived from a Super Like or Flechazo)
  const getMatchType = (match: Match): MatchType => {
    if (match.viaCrush) return "crush";
    if (match.viaSuper) return "super";
  ...
  const stats = {
    dating: matches.filter(m => getMatchType(m) === "dating").length,
    friendship: matches.filter(m => getMatchType(m) === "friendship").length,
    both: matches.filter(m => getMatchType(m) === "both").length,
    super: matches.filter(m => getMatchType(m) === "super").length,
    crush: matches.filter(m => getMatchType(m) === "crush").length,
  ...
  const groupedMatches = {
    ...
    super: matches.filter(m => getMatchType(m) === "super"),
    crush: matches.filter(m => getMatchType(m) === "crush"),
  };
  ...
  const handleCopyMatches = async () => {
    ...
    if (isProfessional) {
      ...
      // Social format
      if (groupedMatches.crush.length > 0) {
        lines.push("💘 ENAMORADOS (Flechazo aceptado):");
        groupedMatches.crush.forEach(m => {
          lines.push(`  ${m.participant1.name} ↔ ${m.participant2.name}`);
        });
        lines.push("");
      }

      if (groupedMatches.super.length > 0) {
        lines.push("⚡ SUPER CONEXIONES (Super Like):");
        groupedMatches.super.forEach(m => {
          lines.push(`  ${m.participant1.name} ↔ ${m.participant2.name}`);
        });
        lines.push("");
      }
  ...
  const getMatchTypeIcon = (type: MatchType) => {
    switch (type) {
      ...
      case "super": return <Zap className="w-4 h-4" />;
      case "crush": return <Heart className="w-4 h-4" />;
    }
  };
  ...
  const getMatchTypeLabel = (type: MatchType) => {
    switch (type) {
      ...
      case "super": return "Super conexión ⚡";
      case "crush": return "Enamorados 💘";
    }
  };
  ...
  const getMatchTypeBadgeStyle = (type: MatchType) => {
    switch (type) {
      ...
      case "super": return "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300 border-amber-200 dark:border-amber-800";
      case "crush": return "bg-rose-100 text-rose-700 dark:bg-rose-950 dark:text-rose-300 border-rose-200 dark:border-rose-800";
    }
  };
  ...
  const getSectionStyle = (type: MatchType) => {
    switch (type) {
      ...
      case "super": return "bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-900";
      case "crush": return "bg-rose-50/50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-900";
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Card className="bg-purple-50/50 dark:bg-purple-950/20 border-purple-200 dark:border-purple-800">
            <CardContent className="p-4 text-center">
              ...
              <div className="text-xs text-purple-600 dark:text-purple-400">Conexiones completas</div>
            </CardContent>
          </Card>
          <Card className="bg-rose-50/50 dark:bg-rose-950/20 border-rose-200 dark:border-rose-800">
            <CardContent className="p-4 text-center">
              ...
              <div className="text-2xl font-bold text-rose-700 dark:text-rose-300">{stats.crush}</div>
              <div className="text-xs text-rose-600 dark:text-rose-400">Enamorados 💘</div>
            </CardContent>
          </Card>
          <Card className="bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800">
            <CardContent className="p-4 text-center">
              <Zap className="w-6 h-6 mx-auto mb-2 text-amber-500" />
              <div className="text-2xl font-bold text-amber-700 dark:text-amber-300">{stats.super}</div>
              <div className="text-xs text-amber-600 dark:text-amber-400">Super conexiones</div>
            </CardContent>
          </Card>
          <Card className="bg-muted/50 border-border">
            <CardContent className="p-4 text-center">
      ...
      {!isProfessional && viewMode === "byMatch" && (
        <div className="space-y-4">
          {/* Enamorados (accepted Flechazo) */}
          {groupedMatches.crush.length > 0 && (
            <Collapsible open={openSections.crush} onOpenChange={() => toggleSection("crush")}>
              <Card className={getSectionStyle("crush")}>
                <CollapsibleTrigger asChild>
                  <CardHeader className="cursor-pointer hover:bg-background/50 transition-colors rounded-t-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-lg">Enamorados 💘</CardTitle>
                          <p className="text-xs text-rose-700/70 dark:text-rose-400/70 mt-1">
                            Originadas al aceptar un Flechazo
                          </p>
                        </div>
                        <Badge variant="secondary" className={getMatchTypeBadgeStyle("crush")}>
                          {groupedMatches.crush.length}
                        </Badge>
                      </div>
                      <ChevronDown className={`w-5 h-5 transition-transform ${openSections.crush ? "rotate-180" : ""}`} />
                    </div>
                  </CardHeader>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <CardContent className="pt-0">
                    <div className="grid gap-3">
                      {groupedMatches.crush.map((match, index) => (
                        <MatchCard key={index} match={match} matchType="crush" />
                      ))}
                    </div>
                  </CardContent>
                </CollapsibleContent>
          ...
          {/* Super connections (Super Like) */}
          {groupedMatches.super.length > 0 && (
            <Collapsible open={openSections.super} onOpenChange={() => toggleSection("super")}>
              <Card className={getSectionStyle("super")}>
                <CollapsibleTrigger asChild>
                  <CardHeader className="cursor-pointer hover:bg-background/50 transition-colors rounded-t-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-lg">Super Conexiones ⚡</CardTitle>
                          <p className="text-xs text-amber-700/70 dark:text-amber-400/70 mt-1">
                            Desbloqueadas por un Super Like
                          </p>
                        </div>
                        <Badge variant="secondary" className={getMatchTypeBadgeStyle("super")}>
                          {groupedMatches.super.length}
                        </Badge>
                      </div>
                      <ChevronDown className={`w-5 h-5 transition-transform ${openSections.super ? "rotate-180" : ""}`} />
                    </div>
                  </CardHeader>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <CardContent className="pt-0">
                    <div className="grid gap-3">
                      {groupedMatches.super.map((match, index) => (
                        <MatchCard key={index} match={match} matchType="super" />
                      ))}
                    </div>
                  </CardContent>
                </CollapsibleContent>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-purple-700/70 dark:text-purple-400/70 mt-1">
                            Amistad + ligue elegidos por ambas personas (sin Super Like)
                          </p>
                        </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex gap-1">
                          {pm.matches.some(m => m.matchType === "crush") && (
                            <Badge variant="secondary" className={getMatchTypeBadgeStyle("crush")}>💘</Badge>
                          )}
...
const MatchCard = ({ match, matchType }: { match: Match; matchType: MatchType }) => {
  const getAvatarGradient = (type: MatchType) => {
    switch (type) {
      ...
      case "super": return "from-amber-400 to-orange-500";
    }
  };
  ...
  const getIconBg = (type: MatchType) => {
    switch (type) {
      ...
      case "super": return "bg-amber-100 dark:bg-amber-900/50";
    }
  };

  const superDetail = match.matchTypes.dating && match.matchTypes.friendship
    ? "Ligue + Amistad"
    : match.matchTypes.dating ? "Ligue" : "Amistad";
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 rounded-xl bg-background/80 border border-border/50 hover:shadow-md transition-shadow animate-fade-in">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${getIconBg(matchType)}`}>
          ...
          {matchType === "super" && <Zap className="w-4 h-4 text-amber-500 animate-pulse" />}
        </div>
        <Avatar className={`w-12 h-12 bg-gradient-to-br ${getAvatarGradient(matchType)} ring-2 ring-background`}>
          <AvatarFallback className="bg-transparent text-white font-semibold">
      ...
      <div className="flex-1">
        ...
        <p className="text-sm text-muted-foreground">
          {matchType === "super" ? `Super conexión ⚡ · ${superDetail}` : "¡Match mutuo!"}
        </p>
      </div>
