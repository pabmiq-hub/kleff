...
import { Card, CardContent, CardHeader, CardTitle } from "@/konektum/ui/card";
import { Badge } from "@/konektum/ui/badge";
import { Alert, AlertDescription } from "@/konektum/ui/alert";
import { 
  Users, 
  UserCheck, 
  UserX,
...
} from "@/konektum/lib/analyticsNormalization";
import {
  PieChart,
  Pie,
  Cell,
...
const LANG_COLORS = ["#0ea5e9", "#14b8a6", "#f59e0b", "#8b5cf6", "#ef4444", "#22c55e", "#6366f1"];
...
const EventAnalytics = ({ participants, selections, matches, tables, originalParticipantsCount, repeatStats }: EventAnalyticsProps) => {
  ...
  const languageStats = useMemo(() => {
    ...
    let withData = 0;
    participants.forEach(p => {
      const langs = normalizeLanguages(p.spoken_languages);
    ...
    const multilingual = participants.filter(p => normalizeLanguages(p.spoken_languages).length > 1).length;
    const avgPerPerson = withData > 0
      ? (participants.reduce((acc, p) => acc + normalizeLanguages(p.spoken_languages).length, 0) / withData).toFixed(1)
      : "0";
    return { data, withData, multilingual, avgPerPerson, total: participants.length };
  }, [participants]);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ========== BLOQUE A.4: IDIOMAS ========== */}
      {languageStats.data.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-2 pb-2 border-b">
            <BarChart3 className="w-5 h-5 text-primary" />
            <h2 className="text-lg font-semibold">Idiomas de los participantes</h2>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2 overflow-hidden">
              <CardHeader className="pb-0 bg-gradient-to-r from-sky-50 to-teal-50">
                <CardTitle className="text-lg flex items-center gap-2">
              ...
              <CardContent className="pt-4">
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={languageStats.data} layout="vertical" margin={{ left: 10, right: 30 }}>
                    ...
                    <Bar dataKey="value" radius={[0, 8, 8, 0]} barSize={24}>
                      {languageStats.data.map((_, index) => (
                        <Cell key={`lang-${index}`} fill={LANG_COLORS[index % LANG_COLORS.length]} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="p-3 rounded-lg bg-sky-50">
                  <div className="text-2xl font-bold text-sky-600">{languageStats.withData}</div>
                  <div className="text-sm text-muted-foreground">
                    Con idiomas declarados de {languageStats.total}
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-teal-50">
                  <div className="text-2xl font-bold text-teal-600">{languageStats.multilingual}</div>
                  <div className="text-sm text-muted-foreground">Hablan más de un idioma</div>
                </div>
                <div className="p-3 rounded-lg bg-muted">
                  <div className="text-2xl font-bold">{languageStats.avgPerPerson}</div>
                  <div className="text-sm text-muted-foreground">Idiomas por persona (media)</div>
                </div>
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {languageStats.data.map(l => (
