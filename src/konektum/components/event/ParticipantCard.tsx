...
import { Tooltip, TooltipContent, TooltipTrigger } from "@/konektum/ui/tooltip";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { normalizePreference } from "@/konektum/lib/analyticsNormalization";
...
interface ParticipantCardProps {
  ...
  onSendSelectionReminder?: (id: string) => void;
  isSendingSelectionReminder?: boolean;
...
const ParticipantCard = ({
  ...
  onSendSelectionReminder,
  isSendingSelectionReminder,
      ...
      <div className="p-4">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-0.5 shrink-0" onClick={(e) => e.stopPropagation()}>
            ...
            {eventStatus === "completed" && participant.email && onSendSelectionReminder && !participant.selection_submitted_at && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-7 w-7 text-muted-foreground hover:text-pink-600"
                    disabled={isSendingSelectionReminder}
                    onClick={() => onSendSelectionReminder(participant.id)}
                  >
                    {isSendingSelectionReminder ? (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                ...
                <TooltipContent>Recordar envío de selecciones</TooltipContent>
              </Tooltip>
            )}

            {participant.email && onSendReminder && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
            ...
            {(participant.email || participant.is_anonymous) && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant="outline"
