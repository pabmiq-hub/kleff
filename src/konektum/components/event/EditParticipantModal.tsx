...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/konektum/ui/dialog";
...
import {
  ...
  PREFERENCES,
  DATING_PREFERENCES,
} from "@/konektum/lib/excelParser";
import WrappedInterestsForm from "@/konektum/components/registration/WrappedInterestsForm";
import { getWrappedQuestions, type WrappedAnswers, type WrappedQuestion } from "@/konektum/lib/wrappedQuestions";
import SocialGameForm from "@/konektum/components/registration/SocialGameForm";
import { Checkbox } from "@/konektum/ui/checkbox";
import { LANGUAGE_OPTIONS, normalizeLanguages } from "@/konektum/lib/languages";
import { type SocialGameAnswers } from "@/konektum/lib/socialGame";
...
const EditParticipantModal = ({
  ...
  const socialGameConfig = normalizeIcebreakers(socialGame).games.who_is_who;
  ...
  // Spoken languages (editable manually by the organizer)
  const [spokenLanguages, setSpokenLanguages] = useState<string[]>(
    normalizeLanguages(participant.spoken_languages)
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setSpokenLanguages(normalizeLanguages(data.spoken_languages));
    };
    load();
    return () => { cancelled = true; };
  }, [isProfessional, participant.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isProfessional) {
      ...
      updateData.spoken_languages = normalizeLanguages(spokenLanguages);
      if (formData.birth_date) {
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 rounded-md border p-3">
          {LANGUAGE_OPTIONS.map((opt) => (
            <label key={opt.code} className="flex items-center gap-2 text-sm font-normal cursor-pointer">
              <Checkbox
                checked={spokenLanguages.includes(opt.label)}
                onCheckedChange={() => toggleLanguage(opt.label)}
              />
              {opt.label}
            </label>
          ))}
