...
import { RichTextEditor } from "@/konektum/ui/rich-text-editor";
...
import { GameModeConfig, EMPTY_GAME_MODE, normalizeGameMode } from "@/konektum/lib/gameMode";
...
import { CustomTableLayout, isCustomTablesEnabled } from "@/konektum/lib/customTableLayout";
...
const EventSettingsEditor = ({
  ...
  const [formSocialGame, setFormSocialGame] = useState<IcebreakersConfig>(normalizeIcebreakers(null));
  ...
  const handleSave = async () => {
    ...
    try {
      const updates: Record<string, any> = {
        ...
        social_game: !isProfessional
          ? serializeIcebreakers({
              ...formSocialGame,
              enabled: formSocialGame.enabled,
              games: {
                ...formSocialGame.games,
                who_is_who: {
                  ...formSocialGame.games.who_is_who,
                  questions: formSocialGame.games.who_is_who.questions.filter(
                    (q) => q.label_es.trim() || q.label_en.trim()
                  ),
                },
              },
            })
          : serializeIcebreakers({ ...formSocialGame, enabled: false }),
    ...
    <>
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {!isProfessional && (
            <IcebreakersEditor value={formSocialGame} onChange={setFormSocialGame} wrappedEnabled={formWrappedEnabled} />
          )}
