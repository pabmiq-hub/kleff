...
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@/konektum/ui/tabs";
import { UserPlus, Database, Lock, EyeOff } from "lucide-react";
...
import AddAnonymousParticipantForm from "./AddAnonymousParticipantForm";
...
import type { Participant } from "@/konektum/lib/excelParser";
...
const AddParticipantTabsModal = ({
  ...
  const [tab, setTab] = useState<"manual" | "anon" | "crm">("manual");
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)} className="flex-1 flex flex-col min-h-0">
          ...
          <TabsContent value="anon" className="flex-1 overflow-y-auto mt-4">
            <AddAnonymousParticipantForm
              onAdd={async (p) => {
                await onAdd(p);
                onClose();
              }}
