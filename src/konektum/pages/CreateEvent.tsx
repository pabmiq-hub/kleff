...
import { useNavigate } from "@/konektum/router";
...
import { useSearchParams } from "@/konektum/router";
import { generateFakeParticipants, type FakeGenConfig } from "@/konektum/lib/fakeParticipantGenerator";
import { DEFAULT_WRAPPED_QUESTIONS, type WrappedQuestion } from "@/konektum/lib/wrappedQuestions";
import { DEFAULT_SOCIAL_GAME_QUESTIONS, type SocialGameConfig } from "@/konektum/lib/socialGame";
import WrappedQuestionsEditor from "@/konektum/components/event/WrappedQuestionsEditor";
import IcebreakersEditor from "@/konektum/components/event/IcebreakersEditor";
...
import { Slider } from "@/konektum/ui/slider";
...
import AddParticipantModal from "@/konektum/components/event/AddParticipantModal";
import AddProfessionalParticipantModal, { ProfessionalParticipant } from "@/konektum/components/event/AddProfessionalParticipantModal";
import EventPreferencesEditor, { EventPreferences, DEFAULT_PREFERENCES } from "@/konektum/components/event/EventPreferencesEditor";
import EventQuotasEditor, { SlotQuota } from "@/konektum/components/event/EventQuotasEditor";
import ProfessionalPreferencesEditor, { ProfessionalPreferences, DEFAULT_PROFESSIONAL_PREFERENCES } from "@/konektum/components/event/ProfessionalPreferencesEditor";
...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/konektum/ui/tooltip";
import GroupRoundsEditor, { GroupRound } from "@/konektum/components/event/GroupRoundsEditor";
import RegistrationFormEditor, { FormField, RegistrationFormConfig, getDefaultFields } from "@/konektum/components/event/RegistrationFormEditor";
import GameModeEditor from "@/konektum/components/event/GameModeEditor";
import { GameModeConfig, EMPTY_GAME_MODE } from "@/konektum/lib/gameMode";
...
const CreateEvent = () => {
  ...
  const [socialGame, setSocialGame] = useState<IcebreakersConfig>(normalizeIcebreakers(null));
  ...
  const handleCreateEvent = async () => {
    ...
    const eventInsertData = {
      ...
      social_game: (eventModule === "social" && (isTestMode || socialGame.enabled)
        ? serializeIcebreakers(
            isTestMode && !socialGame.enabled
              ? {
                  enabled: true,
                  games: {
                    who_is_who: { enabled: true, questions: DEFAULT_SOCIAL_GAME_QUESTIONS },
                    build_yourself: { ...socialGame.games.build_yourself, enabled: true },
                    timeline: { ...socialGame.games.timeline, enabled: true },
                    emoji_story: { ...socialGame.games.emoji_story, enabled: true },
                  },
                }
              : {
                  ...socialGame,
                  games: {
                    ...socialGame.games,
                    // Timeline y Emoji Story dependen de los datos del Modo Wrapped
                    timeline: { ...socialGame.games.timeline, enabled: socialGame.games.timeline.enabled && wrappedEnabled },
                    emoji_story: { ...socialGame.games.emoji_story, enabled: socialGame.games.emoji_story.enabled && wrappedEnabled },
                  },
                },
    ...
    if (participants.length > 0) {
      ...
      if (isTestMode && eventModule === "social" && insertedParticipants && insertedParticipants.length > 0) {
        ...
        const { linkWrappedProfiles } = await import("@/konektum/lib/testModeData");
      ...
      if (participantsError) {
        ...
        const { fillPreliminaryTables } = await import("@/konektum/lib/preliminaryRoundAssign");
    ...
    navigate(`/admin/konektum/eventos/${eventData.id}`);
    setIsLoading(false);
  };
        ...
        <main className="container mx-auto px-4 py-8 max-w-2xl">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Button asChild>
                <Link to="/admin/konektum">Volver al dashboard</Link>
              </Button>
            </CardContent>
          </Card>
        </main>
          ...
          <Card className="animate-fade-in">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                ...
                <IcebreakersEditor value={socialGame} onChange={setSocialGame} wrappedEnabled={wrappedEnabled} />
