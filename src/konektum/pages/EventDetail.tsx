...
import { useParams, Link, useNavigate } from "@/konektum/router";
...
import {
  ...
  DropdownMenuTrigger,
} from "@/konektum/ui/dropdown-menu";
import TableAssignmentModal from "@/konektum/components/event/TableAssignmentModal";
import TableEditorModal from "@/konektum/components/event/TableEditorModal";
import EventAnalytics from "@/konektum/components/event/EventAnalytics";
import EventEngagementInsights from "@/konektum/components/event/EventEngagementInsights";
import EventSettingsTabs from "@/konektum/components/event/EventSettingsTabs";
import { BrandedHeader } from "@/konektum/components/BrandedHeader";
...
import {
  ...
  TooltipProvider,
  TooltipTrigger,
} from "@/konektum/ui/tooltip";
import EmailTemplateEditor, { EmailTemplate } from "@/konektum/components/event/EmailTemplateEditor";
import MatchesDashboard from "@/konektum/components/event/MatchesDashboard";
import SelectionProgress from "@/konektum/components/event/SelectionProgress";
import SelectionsViewer from "@/konektum/components/event/SelectionsViewer";
import EventsViewer from "@/konektum/components/event/EventsViewer";
import EmailManagement from "@/konektum/components/event/EmailManagement";
import InlineEmailEditor from "@/konektum/components/event/InlineEmailEditor";
import ParticipantCard from "@/konektum/components/event/ParticipantCard";
import { normalizeLanguages } from "@/konektum/lib/languages";
import { Popover, PopoverContent, PopoverTrigger } from "@/konektum/ui/popover";
import { Checkbox } from "@/konektum/ui/checkbox";
import { parseEventDate } from "@/konektum/lib/eventDate";
import CloseEventDialog from "@/konektum/components/event/CloseEventDialog";
import ParticipantDetailModal from "@/konektum/components/event/ParticipantDetailModal";
import WaitlistEntryDetailModal from "@/konektum/components/event/WaitlistEntryDetailModal";
import EventCompatibilityTab from "@/konektum/components/event/EventCompatibilityTab";
import EditParticipantModal from "@/konektum/components/event/EditParticipantModal";
import { requiresGameAnswers } from "@/konektum/lib/icebreakers";
import ScheduleEmailDialog from "@/konektum/components/event/ScheduleEmailDialog";
import { useToast } from "@/konektum/hooks/use-toast";
import { Badge } from "@/konektum/ui/badge";
import RoundTimer from "@/konektum/components/event/RoundTimer";
import EventQRCode from "@/konektum/components/event/EventQRCode";
import AddParticipantTabsModal from "@/konektum/components/event/AddParticipantTabsModal";
import type { CRMPickerParticipant } from "@/konektum/components/event/AddParticipantFromCRM";
import AddProfessionalParticipantModal, { type ProfessionalParticipant as ModalProfessionalParticipant } from "@/konektum/components/event/AddProfessionalParticipantModal";
import ExcelPreviewModal from "@/konektum/components/event/ExcelPreviewModal";
import ExclusionsManager from "@/konektum/components/event/ExclusionsManager";
import InclusionsManager from "@/konektum/components/event/InclusionsManager";
import { loadInclusions, computeInclusionGroups, buildInclusionGroupIndex } from "@/konektum/lib/inclusions";
import { parseExcelFile, Participant } from "@/konektum/lib/excelParser";
import { exportMatchesToCSV, exportMatchesToExcel } from "@/konektum/lib/exportMatches";
import { exportTableAssignmentsToExcel } from "@/konektum/lib/exportTableAssignments";
import { supabase } from "@/konektum/supabase";
import { useGlobalParticipants } from "@/konektum/hooks/useGlobalParticipants";
import { useFeatures } from "@/konektum/hooks/useFeatures";
import { FeatureGate } from "@/konektum/components/FeatureGate";
import { generateB2BTables, b2bToStandardTableFormat, validateB2BParticipants, type ProfessionalParticipant as B2BParticipant } from "@/konektum/lib/b2bTableGenerator";
import {
  GameModeConfig,
  normalizeGameMode,
...
interface EventData {
  ...
  game_mode: import("@/konektum/lib/gameMode").GameModeConfig | null;
  custom_tables: import("@/konektum/lib/customTableLayout").CustomTableLayout | null;
...
interface Match {
  ...
  /** True when the match only exists thanks to a Super Like */
  viaSuper?: boolean;
  /** True when the match comes from an accepted Flechazo ("Enamorados") */
  viaCrush?: boolean;
}
...
const EventDetail = () => {
  ...
  const [sendingSelectionReminderFor, setSendingSelectionReminderFor] = useState<string | null>(null);
  ...
  const loadEventData = async () => {
    ...
    if (selectionsData && participantsData) {
      ...
      const acceptedCrushPairs = new Set<string>(); // pair keys with an accepted Flechazo

      // Only ACCEPTED flechazos produce a match (pending/declined ones must not).
      // An accepted flechazo is mutual by definition, so we synthesize the
        ...
        .forEach((cr: any) => {
          ...
          const upsert = (from: string, to: string) => {
            ...
            if (idx === undefined) {
              ...
              augIndex.set(k, augmentedSelections.length - 1);
            } else {
          ...
          acceptedCrushPairs.add([cr.requester_id, cr.target_id].sort().join('-'));
        });

      const mutualMatches: Match[] = [];
      const processed = new Set<string>();

      const rawTypeOf = (a: string, b: string): string | null => {
        const raw = filteredSelections.find(s => s.selector_id === a && s.selected_id === b);
        return raw ? (raw.selection_type || 'friendship') : null;
      };
      ...
      augmentedSelections.forEach(sel => {
        ...
        if (reverse) {
          ...
          if (p1 && p2) {
            const hasType = (t: string | null, kind: 'friendship' | 'dating') =>
              t === kind || t === 'both';

            // Augmented (Super Like / Flechazo aware) result
            const augFriendship = hasType(sel.selection_type || 'friendship', 'friendship')
              && hasType(reverse.selection_type || 'friendship', 'friendship');
            let augDating = hasType(sel.selection_type || 'friendship', 'dating')
              && hasType(reverse.selection_type || 'friendship', 'dating');

            // Raw (real, declared) selections without any Super Like / Flechazo boost
            const raw1 = rawTypeOf(sel.selector_id, sel.selected_id);
            const raw2 = rawTypeOf(reverse.selector_id, reverse.selected_id);
            const rawFriendship = hasType(raw1, 'friendship') && hasType(raw2, 'friendship');
            let rawDating = hasType(raw1, 'dating') && hasType(raw2, 'dating');

            // A romance match requires bilateral dating-preference compatibility
            const datingCompatible = areDatingCompatible(
              p1.dating_preference, p1.gender, p2.dating_preference, p2.gender
            );
            const viaCrush = acceptedCrushPairs.has(key);

            // An accepted Flechazo is an explicit, validated romance match
            if (!datingCompatible && !viaCrush) {
              augDating = false;
              rawDating = false;
            }

            const viaSuper = !viaCrush
              && ((augFriendship && !rawFriendship) || (augDating && !rawDating));

            const matchTypes = { friendship: augFriendship, dating: augDating || viaCrush };

            if (matchTypes.friendship || matchTypes.dating) {
              mutualMatches.push({ participant1: p1, participant2: p2, matchTypes, viaSuper, viaCrush });
            }
          }
          processed.add(key);
        }
  ...
  const handleToggleCheckin = async (participantId: string, currentStatus: boolean) => {
    if (!currentStatus) {
      ...
      if (id) {
        ...
        if (p) {
          const { assignParticipantsToPreliminaryTables } = await import("@/konektum/lib/preliminaryRoundAssign");
  ...
  const handleSendCode = async (participantId: string) => {
    ...
    try {
      ...
      if (!participant?.email && !(participant as any)?.is_anonymous) {
      ...
      toast({
        ...
        description: data?.emailSent
          ...
          : data?.verificationCode
            ? `Código de acceso: ${data.verificationCode} (compártelo manualmente)`
  ...
  const handleSendBulkCodes = async () => {
    // Send codes to all participants with email (or anonymous, code only) but no code yet
    const pendingParticipants = participants.filter(
      p => !p.verification_code && (p.email || (p as any).is_anonymous)
    );
  ...
  const handleAddParticipant = async (participant: Participant) => {
    ...
    const { data: newParticipant, error } = await supabase
      ...
      .insert({
        ...
        is_anonymous: participant.isAnonymous || false,
      })
      .select()
      .single();
    ...
    } else if (newParticipant.is_anonymous) {
      // Anonymous participants have no email: generate the access code anyway
      try {
        const { data } = await supabase.functions.invoke('generate-and-send-code', {
          body: { eventId: id, participantId: newParticipant.id }
        });
        if (data?.verificationCode) {
          ...
          toast({
            ...
            description: `Código para ${participant.name}: ${data.verificationCode} (compártelo manualmente)`,
          });
        }
      } catch (e) {
        console.error('Error generating anonymous code:', e);
      }
    }
  };
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('register-participant', {
        body: {
          ...
          spokenLanguages: Array.isArray((entry as any).spoken_languages) ? (entry as any).spoken_languages : undefined,
          fromWaitlist: true,
        }
      });
  ...
  const handleCopyEvent = async (withParticipants: boolean) => {
    ...
    try {
      ...
      navigate(`/admin/konektum/eventos/${newEvent.id}`);
    } catch (error: any) {
  ...
  const handleTableEditorSave = async (updatedRoundData: any) => {
    ...
    if (updatedRoundData?.__preliminary || updatedRoundData?.round === 0) {
      ...
      const newTables: any[][] = updatedRoundData.tables || [];
      const newConfirmations = updatedRoundData.confirmations ?? eventData.preliminary_round.confirmations ?? {};
      // Drop dismissed indexes that no longer exist and un-dismiss tables whose
      // members are no longer all marked as absent.
      const newDismissed = (eventData.preliminary_round.dismissed_tables || []).filter((i: number) => {
        if (i >= newTables.length) return false;
        const t = newTables[i] || [];
        return t.length > 0 && t.every((p: any) => newConfirmations[p.id] === false);
      });
      const updatedPrelim = {
        ...eventData.preliminary_round,
        tables: newTables,
        confirmations: newConfirmations,
        dismissed_tables: newDismissed,
      };
    ...
    .filter(p => {
      ...
      if (!isProfessionalEvent) {
        ...
        if (filterLanguages.length > 0) {
          const pLangs = normalizeLanguages((p as any).spoken_languages);
          if (!filterLanguages.every(l => pLangs.includes(l))) return false;
        }
      } else {
  ...
  const uniqueLanguages = [...new Set(
    participants.flatMap(p => normalizeLanguages((p as any).spoken_languages))
      ...
      <main className="container mx-auto px-4 py-8">
            ...
            <div className="flex-1">
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    const { backfillTestEventData } = await import("@/konektum/lib/testModeData");
                    ...
                    <div className="space-y-3 pt-2 border-t">
                      ...
                      <div className={`${showFilters ? "flex" : "hidden"} sm:flex flex-wrap items-center gap-2 sm:gap-3`}>
                              ...
                              {uniqueLanguages.length > 0 && (
                                <Popover>
                                  <PopoverTrigger asChild>
                                    <Button
                                      variant={filterLanguages.length > 0 ? "default" : "outline"}
                                      size="sm"
                                      className="h-8 px-2 text-xs sm:text-sm font-normal justify-between col-span-2 sm:col-span-1"
                                    >
                                      <span className="flex items-center gap-1">
                                        <Languages className="w-3.5 h-3.5" />
                                        {filterLanguages.length > 0 ? `Idiomas (${filterLanguages.length})` : "Idioma"}
                                      </span>
                                    </Button>
                                  </PopoverTrigger>
                                  <PopoverContent align="start" className="w-56 p-2">
                                    <p className="text-xs text-muted-foreground px-1 pb-2">
                                      Muestra participantes que hablen todos los idiomas marcados
                                    </p>
                                    <div className="space-y-1">
                                      {uniqueLanguages.map(lang => {
                                        const checked = filterLanguages.includes(lang);
                                        return (
                                          <label
                                            key={lang}
                                            className="flex items-center gap-2 px-1 py-1.5 rounded-md hover:bg-muted cursor-pointer text-sm"
                                          >
                                            <Checkbox
                                              checked={checked}
                                              onCheckedChange={(v) =>
                                                setFilterLanguages(prev =>
                                                  v ? [...prev, lang] : prev.filter(l => l !== lang)
                                                )
                                              }
                                            />
                                            <span>{lang}</span>
                        ...
                        {((isProfessionalEvent && (filterEntityType !== "all" || filterSector !== "all" || filterCheckin !== "all" || searchTerm)) ||
                          (!isProfessionalEvent && (filterGender !== "all" || filterAgeRange !== "all" || filterPreferredAgeRange !== "all" || filterPreference !== "all" || filterLanguages.length > 0 || filterCheckin !== "all" || searchTerm))) && (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => {
                  ...
                  <div className="grid gap-3">
                    {filteredParticipants.map((participant, index) => (
                      <ParticipantCard
                        ...
                        onSendSelectionReminder={async (pId) => {
                          setSendingSelectionReminderFor(pId);
                          try {
                            await handleSendReminder([pId], "selection");
                          } finally {
                            setSendingSelectionReminderFor(null);
                          }
                        }}
                        isSendingSelectionReminder={sendingSelectionReminderFor === participant.id}
        ...
        {selectedWaitlistEntry && (
          <WaitlistEntryDetailModal
            ...
            socialGame={(eventData as any)?.social_game}
            isProfessional={eventData?.module === "professional"}
          />
        )}
        ...
        {editingParticipant && (
          <EditParticipantModal
            ...
            socialGameEnabled={requiresGameAnswers((eventData as any)?.social_game)}
        ...
        <TableEditorModal
          ...
          isPreliminary={!!(editingRoundData as any)?.__preliminary || editingRoundData?.round === 0}
          confirmations={eventData?.preliminary_round?.confirmations || {}}
