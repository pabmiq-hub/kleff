// @ts-nocheck
// Ported from the original Konektum edge function `update-selection`.
...
serve(async (req) => {
  ...
  try {
    ...
    // Only block when the selection window has been explicitly closed.
    // An event with status 'completed' is normal after the rounds finish —
    // participants must still be able to edit until selection_closed_at is set.
    ...
    if (finalType === 'dating' || finalType === 'both') {
      // Unknown orientation on either side → treat as compatible (cannot prove otherwise).
