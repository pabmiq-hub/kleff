// @ts-nocheck
// Ported from the original Konektum edge function `submit-game-vote`.
...
serve(async (req) => {
  ...
  try {
    ...
    const whoIsWhoEnabled = !!cfg?.enabled && (cfg?.games ? !!cfg.games?.who_is_who?.enabled : true);
    if (!event || !whoIsWhoEnabled) {
    ...
    if (earned.length > 0) {
      const { error: rewardError } = await supabase.from("game_rewards").upsert(
        earned.map((type) => ({
          event_id: eventId,
