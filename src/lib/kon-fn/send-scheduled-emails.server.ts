// @ts-nocheck
// Ported from the original Konektum edge function `send-scheduled-emails`.
...
const DEFAULT_PROFESSIONAL_TEMPLATE: ProfessionalEmailTemplate = {
  withConnections: {
    ...
    closing: "Le animamos a ponerse en contacto con estas empresas para explorar posibles sinergias y oportunidades de negocio.",
    signature: "Atentamente,\nEl equipo organizador",
  },
  withoutConnections: {
    subject: "Gracias por participar en {{evento}}",
    greeting: "Estimado/a {{nombre}},",
    message: "Agradecemos sinceramente su participación en nuestro evento de networking profesional {{evento}}. Aunque en esta ocasión no se han generado conexiones específicas, le mantendremos informado de futuras oportunidades.",
  ...
  try {
    ...
    for (const event of eventsToProcess || []) {
      ...
      const { data: tableAssignments } = isProfessional 
        ? await supabase.from("table_assignments").select("participant_id, table_number, round").eq("event_id", event.id)
        : { data: null };

      // Build connections for professional events
        ...
        for (const [_, participantIds] of tableGroups) {
          ...
          for (let i = 0; i < participantIds.length; i++) {
            for (let j = i + 1; j < participantIds.length; j++) {
              ...
              if (!existing1.some(c => c.contactPerson === p2.name)) {
                existing1.push({
              ...
              if (!existing2.some(c => c.contactPerson === p1.name)) {
                existing2.push({
