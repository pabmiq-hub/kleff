...
import ParticipantSelect from "@/konektum/pages/ParticipantSelect";

export const Route = createFileRoute("/$eventSlug/seleccion")({
  loader: ({ params }) => resolveKonEvent({ data: { slug: params.eventSlug } }),
  head: ({ loaderData }) => {
    const name = loaderData?.name ?? "Evento";
    const title = `Selecciones · ${name} · KLEFF`;
    const description = `Selecciones del evento ${name} organizado por KLEFF.`;
    return {
      meta: [
        { title },
