    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button onClick={handleBulkTranslate} disabled={bulkTranslating || posts.length === 0} variant="outline" className="border-cream/20 text-cream hover:bg-cream/10">
      ...
      <div className="rounded-lg border border-cream/10 bg-cream/[0.03] overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {posts.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-cream/60">Aún no hay posts. Pulsa "Importar de WordPress" para empezar.</td>
              </tr>
            ) : (
              posts.map((p) => (
                <tr key={p.id} className="border-t border-cream/10 hover:bg-cream/[0.02]">
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-cream/50 mt-0.5">/{p.slug} · {new Date(p.published_at).toLocaleDateString("es-ES")}</div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <Link to="/admin/blog/$id" params={{ id: p.id }} className="inline-flex items-center justify-center h-8 w-8 rounded-md text-cream/80 hover:text-cream hover:bg-cream/10" title="Editar"><Pencil className="h-3.5 w-3.5" /></Link>
                      <Button size="sm" variant="ghost" onClick={() => handleTranslate(p.id)} disabled={translatingId === p.id || bulkTranslating} className="text-cream/80 hover:text-cream hover:bg-cream/10 h-8 px-2">
                        {translatingId === p.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Languages className="h-3.5 w-3.5" />}
                      </Button>
                      <a href={`/en/${p.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center h-8 w-8 rounded-md text-cream/80 hover:text-cream hover:bg-cream/10" title="Ver post en inglés"><ExternalLink className="h-3.5 w-3.5" /></a>
                    </div>
                  </td>
                </tr>
              ))
