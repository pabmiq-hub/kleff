...
function AdminBlog() {
  ...
  if (loading) {
    return <div className="flex items-center gap-2 p-6 text-ink/60"><Loader2 className="h-4 w-4 animate-spin" /> Cargando blog…</div>;
  }
    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-2 flex-wrap">
          <Link to="/admin/blog/new" className="inline-flex items-center gap-2 bg-coral hover:bg-coral/90 text-ink rounded-md px-3 py-2 text-sm font-medium">
            <Plus className="h-4 w-4" /> Nuevo post
          </Link>
          <Button onClick={handleImport} disabled={importing} variant="outline" className="border-ink/20 text-ink hover:bg-ink/10">
            {importing ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Download className="h-4 w-4 mr-2" />}
            Importar de WordPress
          </Button>
          <Button onClick={handleBulkTranslate} disabled={bulkTranslating || posts.length === 0} variant="outline" className="border-ink/20 text-ink hover:bg-ink/10">
            {bulkTranslating ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Languages className="h-4 w-4 mr-2" />}
          ...
          >
            {mirroring ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <ImageIcon className="h-4 w-4 mr-2" />}
            Rehospedar imágenes
          </Button>
        </div>
      </header>
      ...
      {progress && (
        <div className="rounded-lg border border-ink/10 bg-ink/5 p-4">
          ...
          <div className="h-2 rounded-full bg-ink/10 overflow-hidden">
            <div className="h-full bg-coral transition-all" style={{ width: `${(progress.done / progress.total) * 100}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="rounded-lg border border-ink/10 bg-white overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {posts.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-ink/60">Aún no hay posts. Pulsa "Importar de WordPress" para empezar.</td>
              </tr>
            ) : (
              posts.map((p) => (
                <tr key={p.id} className="border-t border-ink/10 hover:bg-cream/[0.02]">
                  <td className="px-4 py-3">
                    <Link to="/admin/blog/$id" params={{ id: p.id }} className="font-medium text-ink hover:text-coral">{p.title_en ?? p.slug}</Link>
                    <div className="text-xs text-ink/50 mt-0.5">/{p.slug} · {new Date(p.published_at).toLocaleDateString("es-ES")}</div>
                  </td>
                  <td className="px-4 py-3 text-center">{p.title_en ? <CheckCircle2 className="h-4 w-4 text-emerald-400 inline" /> : <AlertCircle className="h-4 w-4 text-amber-400 inline" />}</td>
                  <td className="px-4 py-3 text-center">{p.hasEs ? <CheckCircle2 className="h-4 w-4 text-emerald-400 inline" /> : <AlertCircle className="h-4 w-4 text-amber-400 inline" />}</td>
                  <td className="px-4 py-3 text-center">{p.hasCa ? <CheckCircle2 className="h-4 w-4 text-emerald-400 inline" /> : <AlertCircle className="h-4 w-4 text-amber-400 inline" />}</td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      <Link to="/admin/blog/$id" params={{ id: p.id }} className="inline-flex items-center justify-center h-8 w-8 rounded-md text-ink/80 hover:text-ink hover:bg-ink/10" title="Editar"><Pencil className="h-3.5 w-3.5" /></Link>
                      <Button size="sm" variant="ghost" onClick={() => handleTranslate(p.id)} disabled={translatingId === p.id || bulkTranslating} className="text-ink/80 hover:text-ink hover:bg-ink/10 h-8 px-2">
                        {translatingId === p.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Languages className="h-3.5 w-3.5" />}
                      </Button>
                      <a href={`/en/${p.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center h-8 w-8 rounded-md text-ink/80 hover:text-ink hover:bg-ink/10" title="Ver post en inglés"><ExternalLink className="h-3.5 w-3.5" /></a>
                    </div>
                  </td>
                </tr>
              ))
...
function Stat({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="rounded-lg border border-ink/10 bg-white p-4">
      ...
      <div className="text-2xl font-display font-semibold text-ink mt-1">{value}</div>
    </div>
  );
}