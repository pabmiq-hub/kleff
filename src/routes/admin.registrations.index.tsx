            ...
            {loading ? (
              ...
              <tr key={f.id} className="border-t border-cream/10 hover:bg-cream/[0.02]">
                ...
                <td className="px-4 py-3 text-center">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${f.is_published ? "bg-emerald-500/20 text-emerald-300" : "bg-cream/10 text-cream/60"}`}>{f.is_published ? "Publicado" : "Borrador"}</span>
                </td>
                <td className="px-4 py-3 text-center">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    {f.is_published && <a href={`/inscripcion/${f.slug}`} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center h-8 w-8 rounded-md text-cream/80 hover:text-cream hover:bg-cream/10" title="Ver pública"><ExternalLink className="h-3.5 w-3.5" /></a>}
