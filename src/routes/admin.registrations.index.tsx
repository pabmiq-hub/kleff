    ...
    <div className="space-y-6">
      <header className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="bg-white border-ink/15 text-ink">
            <DialogHeader>
              <DialogTitle>Nueva inscripción</DialogTitle>
            </DialogHeader>
            <div className="space-y-3">
              <div>
                <label className="text-xs uppercase tracking-wider text-ink/60 mb-1 block">Slug (URL)</label>
                <Input value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-"))} placeholder="torneo-catan-2026" className="bg-white border-ink/15 text-ink" />
                <p className="text-xs text-ink/50 mt-1">URL: /inscripcion/{slug || "tu-slug"}</p>
              </div>
              <div>
                <label className="text-xs uppercase tracking-wider text-ink/60 mb-1 block">Título</label>
                <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Inscripción Torneo Catan" className="bg-white border-ink/15 text-ink" />
              </div>
            </div>
            <DialogFooter>
            ...
            {loading ? (
              ...
              <tr key={f.id} className="border-t border-ink/10 hover:bg-cream/[0.02]">
                <td className="px-4 py-3">
                  <Link to="/admin/registrations/$id" params={{ id: f.id }} className="font-medium text-ink hover:text-coral">{f.title_es || f.slug}</Link>
                  <div className="text-xs text-ink/50 mt-0.5">/inscripcion/{f.slug}{f.external_mode ? ` · externo (${f.external_mode})` : ""}</div>
                </td>
                <td className="px-4 py-3 text-center">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${f.is_published ? "bg-emerald-500/20 text-emerald-300" : "bg-ink/10 text-ink/60"}`}>{f.is_published ? "Publicado" : "Borrador"}</span>
                </td>
                <td className="px-4 py-3 text-center">
