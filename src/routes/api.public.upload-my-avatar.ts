  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // The browser already compresses to WebP under 1 MB; this is just a
        // safety net for the final (already optimised) file.
        if (file.size > 3 * 1024 * 1024) return new Response("File too large", { status: 413 });
